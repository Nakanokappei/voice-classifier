...
1. 使用 OpenAI embedding 模型对每一行（去重后）进行向量化。
...
- `OPENAI_API_KEY`（必填）
- `OPENAI_EMBEDDING_MODEL`（可选覆盖）
- `OPENAI_REQUEST_TIMEOUT`（秒，默认 60）
...
| 症状 | 解决方案 |
|---|---|
| `OPENAI_API_KEY is not set` | 在 `.env` 填入或设置环境变量。 |
...
- 工具会将文字发送至 OpenAI Embeddings 及（可选）Chat Completions。
  敏感数据请先在本地进行脱敏处理。
